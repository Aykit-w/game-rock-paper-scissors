...
class RockPaperScissorsApp(tk.Tk):
    def __init__(self):
        ...
        self.ties_label = tk.Label(frame_stats, text="Ничьи: 0", font=("Helvetica", 12), fg="white", bg="#153ddc") # Новая метка для ничьих
        self.ties_label.pack(pady=2)
    ...
    def update_stats(self, game_outcome):
        if game_outcome == "Победа!":
            self.wins += 1
            self.current_streak += 1
            if self.current_streak > self.longest_streak:
                self.longest_streak = self.current_streak
        elif game_outcome == "Проигрыш":
            self.losses += 1
            self.current_streak = 0
        elif game_outcome == "Ничья!":
            self.ties += 1
            self.current_streak = 0
        ...
        self.current_streak_label.config(text=f"Текущая серия побед: {self.current_streak}")
        self.longest_streak_label.config(text=f"Рекорд серии побед: {self.longest_streak}")
